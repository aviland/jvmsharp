...
namespace jvmsharp.native.java.lang
{
    class Class
    {
        ...
        private static void getPrimitiveClass(ref Frame frame)
        {
            rtda.heap.Object nameObj = frame.LocalVars().GetRef(Convert.ToUInt32(0));
            string name = StringPool.GoString(ref nameObj);

            ClassLoader loader = frame.method.Class().loader;
        ...
        private static void getName0(ref Frame frame)
        {
            ...
            var nameObj = StringHelper.JString(ref clas.loader, name);
            frame.OperandStack().PushRef(nameObj);
        }

        private static void desiredAssertionStatus0(ref Frame frame)
        {
